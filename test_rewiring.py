...
def run_diff_homophs(rewire_type='tc_two', n_avg=10, fm=.1, remove_neighbor=False, n_iter=600000):
    """
    ...
    if fm <= .11:
        savename = sname + 'diff_homophs_{}.p'.format(rewire_type)
    elif fm >= .49:
        savename = sname + 'diff_homophs_eqsize_{}.p'.format(rewire_type)
    else:
        savename = sname + 'diff_homophs_fm{}_{}.p'.format(int(100*fm), rewire_type)
    ...
    for i, sa in enumerate(bias):
        ...
        for j, sb in enumerate(bias):
            ...
            conv_d = 0
            for _ in range(n_avg):
                _, t, _, corr, conv = apm.run_rewiring(N, fm, cval, hms, p0, n_iter=n_iter, track_steps=n_iter, rewire_type=rewire_type, remove_neighbor=remove_neighbor)
                _, t0, _, corr0, _ = apm.run_rewiring(N, fm, 0, hms, p0, n_iter=n_iter, track_steps=n_iter, rewire_type=rewire_type, remove_neighbor=remove_neighbor)
            ...
            conv_d /= n_avg
            _save_results(taa, tbb, rho_a, rho_b, taa0, tbb0, rho_a0, rho_b0, results, i, j, savename, rewire_type, conv_d)
...
def plot_rewiring_heatmap(results, rewire_type, savename):
    ...
    suptitles = {'pa_one': 'PA 1', 'pa_two': 'PA 2', 'tc_one': 'TC 1', 'tc_two': 'TC 2', 'tc_four': 'TC 4'}
...
if __name__=='__main__':
    ...
    parser.add_argument('--rewire_type', type=str, default='tc_one')
    ...
    if pargs.analysis == 'eqhomo':
        run_equalsize(rewire_type=pargs.rewire_type, n_avg=pargs.n_avg, remove_neighbor=pargs.remove_neighbor, n_iter=pargs.n_iter)
    elif pargs.analysis == 'diffhomo':
        run_diff_homophs(rewire_type=pargs.rewire_type, n_avg=pargs.n_avg, fm=pargs.fm, remove_neighbor=pargs.remove_neighbor, n_iter=pargs.n_iter)

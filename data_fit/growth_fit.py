...
def grow_gradloglikbase_jointmulti(sa, sb, c, Paa, Pbb, cnt, na):
    """
    Each vector contains the log derivaties of Maa, Mab, Man, Mbb, Mba, Mbn
    """
    Dsa = [1/sa, 1/(sa-1), (2 - 4*na + 2*c*(-1 + 2*na - Paa + Pbb))/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sa) + 2*(na + sa - 2*na*sa)), 0, 0, 0]
    Dsb = [0, 0, 0, 1/(sb-1), 1/sb, (2 - 4*na + 2*c*(-1 + 2*na - Paa + Pbb))/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sb) + 2*(-1 + na + sb - 2*na*sb))]
    Dc = [0, 0, 0, 0, 0, 0]
    Dc[0] = (1 - 2*na + Paa - Pbb)/(2*na + c*(1 - 2*na + Paa - Pbb))
    Dc[1] = (-1 + 2*na - Paa + Pbb)/(2 - 2*na + c*(-1 + 2*na - Paa + Pbb))
    Dc[2] =((-1 + 2*na - Paa + Pbb)*(-1 + 2*sa))/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sa) + 2*(na + sa - 2*na*sa))
    ...
    Lsa = sum([x*p for x, p in zip(cnt, Dsa)]) #real likelihood
    Lsb = sum([x*p for x, p in zip(cnt, Dsb)])
    Lc = sum([x*p for x, p in zip(cnt, Dc)])
...
def grow_hessloglik_base(sa, sb, c, Paa, Pbb, cnt, na):
    ...
    """
    h = np.zeros((3,3))
    x1, x2, x3, x4, x5, x6 = cnt

    #D[dmaasa*x1 + dmabsa*x2 + dmansa*x3, sa] // Simplify // FortranForm
    h[0,0] = -(x1/sa**2) - x2/(-1 + sa)**2 - (4*(1 - 2*na + c*(-1 + 2*na - Paa + Pbb))**2*x3)/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sa) + 2*(na + sa - 2*na*sa))**2
    h[0,1] = 0
    #D[dmansa, c] // Simplify // FortranForm
    h[0,2] = x3*(2*(-1 + 2*na - Paa + Pbb))/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sa) + 2*(na + sa - 2*na*sa))**2

    h[1,0] = 0
    #D[dmbbsb*x5 + dmbasb*x4 + dmbnsb*x6, sb] // Simplify // FortranForm
    h[1,1] = -(x5/sb**2) - x4/(-1 + sb)**2 -(4*(1 - 2*na + c*(-1 + 2*na - Paa + Pbb))**2*x6)/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sb) +2*(-1 + na + sb - 2*na*sb))**2
    #D[dmbnsb, c] // Simplify // FortranForm
    h[1,2] = x6*(2 - 4*na + 2*Paa - 2*Pbb)/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sb) +2*(-1 + na + sb - 2*na*sb))**2

    #D[dmanc, sa] // Simplify // FortranForm
    h[2,0] = x3*(2*(-1 + 2*na - Paa + Pbb))/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sa) + 2*(na + sa - 2*na*sa))**2
    #D[dmbnc, sb] // Simplify // FortranForm
    h[2,1] = x6*(2 - 4*na + 2*Paa - 2*Pbb)/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sb) +2*(-1 + na + sb - 2*na*sb))**2
    #D[dmaac*x1 + dmabc*x2 + dmanc*x3 + dmbbc*x5 + dmbac*x4 + dmbnc*x6,c] // Simplify // FortranForm
    h[2,2] = (1 - 2*na + Paa - Pbb)**2*(-(x1/(2*na + c*(1 - 2*na + Paa - Pbb))**2) - x2/(2 - 2*na + c*(-1 + 2*na - Paa + Pbb))**2 - ((1 - 2*sa)**2*x3)/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sa) + 2*(na + sa - 2*na*sa))**2 - x5/(2 - 2*na + c*(-1 + 2*na - Paa + Pbb))**2 - x4/(2*na + c*(1 - 2*na + Paa - Pbb))**2 - ((1 - 2*sb)**2*x6)/(c*(-1 + 2*na - Paa + Pbb)*(-1 + 2*sb) +  2*(-1 + na + sb - 2*na*sb))**2)
    return h
